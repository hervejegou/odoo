...
class Auth0OAuthLogin(OAuthLogin):
    ...
    def _validate(self, authorization_code, provider_id):
        ...
        post_data_encode = post_data_encode.encode('utf-8')

        # the validation endpoint here must be HTTPs because this is where we are using our client secret
        req = urllib.request.Request(provider['validation_endpoint'], data=post_data_encode)
        ...
        try:
            ...
            if mime_type != 'application/json' or resp.code != 200:
                _logger.error(
                    'API call made to %s did not return the expected response' % provider['validation_endpoint'])
                return False
    ...
    def get_profile_data(request_obj, jwt_secret=None):
        ...
        if jwt_secret is None:
            provider = request_obj.env['auth.oauth.provider'].sudo().search_read(
                [('id', '=', request_obj.session['auth0.provider_id'])])
            if not len(provider):
                return False
            provider = provider[0]
        ...
        if not jwt_secret:
            data = jwt.decode(request_obj.session['auth0.id_token'], options={"verify_signature": False},
                              algorithms="HS256")
        else:
            data = jwt.decode(request_obj.session['auth0.id_token'], key=jwt_secret, algorithms="HS256")
        return data
